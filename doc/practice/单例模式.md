...
但是，如果我们要创建很多的 div，这里的 `return instance = this;` 就需要删掉。
...
把 a 和 b 都定义为 namespace1 的属性，减少了变量和全局作用域打交道的机会，还可以动态地创建命名空间：
...
_创建对象_ 和 _管理单例_ 的职责被分布在两个不同的方法中，两个方法组合起来才具有单例模式的威力。
